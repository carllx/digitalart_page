...
import { PerspectiveCamera, Raycaster, Vector2, Scene, OrthographicCamera, WebGLRenderer, Vector3, Math as Math$1 } from 'three';
import { createContext, memo, useRef, useState, useEffect, useCallback, useLayoutEffect, createElement, useContext, useMemo } from 'react';
...
function renderGl(state, timestamp, repeat, runGlobalEffects) {
  ...
  // Run global effects
  if (runGlobalEffects) globalEffects.forEach(effect => effect(timestamp) && repeat++); // Decrease frame count

  state.current.frames = Math.max(0, state.current.frames - 1);
  repeat += !state.current.invalidateFrameloop ? 1 : state.current.frames; // Run local effects

  state.current.subscribers.forEach(fn => fn(state.current, timestamp)); // Render content
...
let catalogue = {};
const extend = objects => catalogue = _extends({}, catalogue, objects);
...
export { Canvas, addEffect, apply, applyProps, createPortal, extend, invalidate, render, unmountComponentAtNode, useRender, useResource, useThree, useUpdate };
